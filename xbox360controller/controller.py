...
ControllerEvent = namedtuple(
    'Event', ['time', 'type', 'number', 'value', 'is_init'])
...
class Xbox360Controller:
    ...
    def __init__(self, index=0, axis_threshold=0.2, raw_mode=False):
        ...
        try:
            ...
            raise Exception('controller device with index {index} '
        ...
        if raw_mode:
            ...
            self.axes = [self.axis_l, self.axis_r, self.hat,
            ...
            self.buttons = [
                ...
                self.button_trigger_l, self.button_trigger_r,
                self.button_select, self.button_start, self.button_mode,
                self.button_thumb_l, self.button_thumb_r]
    ...
    def _get_event_file(self):
        event_file_sysfs = glob('/sys/class/input/js{idx}/device/event*'
    ...
    def process_event(self, event):
        if event.type == JS_EVENT_BUTTON:
            try:
                ...
                if button._value and button.when_pressed is not None \
                        and callable(button.when_pressed):
                    button.when_pressed(button)

                if not button._value and button.when_released is not None \
    ...
    def driver_version(self):
        ...
        version_dev = (
            version_dev >> 16, (version_dev >> 8) & 0xff, version_dev & 0xff)

        buf = array('i', [0])
        ioctl(self._event_file, EVIOCGVERSION, buf)
        version_ev = struct.unpack('i', buf.tobytes())[0]
        version_ev = (
            version_ev >> 16, (version_ev >> 8) & 0xff, version_ev & 0xff)
